...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "persona scope"
      decision: "produkt dwustronny; persona główna = kibic, drugorzędna = administrator"
    - topic: "status quo / koszt dziś"
      decision: "zastępuje Facebooka — aktualności giną w feedzie, brak archiwum"
    - topic: "insight vs social media"
      decision: "skupienie aktualności + forum w jednym klubowym miejscu; AI-generowanie wpisów jako wyróżnik"
    - topic: "auth strategy"
      decision: "OAuth (Google/Facebook); brak haseł w aplikacji"
    - topic: "access model"
      decision: "aktualności publiczne; całe forum za logowaniem; role gość/kibic/administrator"
  frs_drafted: 16
  quality_check_status: accepted
---

# Shape Notes — Płomień Kostrze

> Seed: strona internetowa drużyny piłkarskiej Płomień Kostrze do dzielenia się
> aktualnościami z kibicami. Generowanie wpisów aktualności z wydarzeń meczowych
> (z akceptacją administratora), ręczne wpisy, proste forum, konta użytkowników.

## Vision & Problem Statement

Kibice drużyny piłkarskiej Płomień Kostrze nie mają jednego, klubowego miejsca,
w którym mogliby śledzić aktualności (wyniki, wydarzenia meczowe, życie klubu) i
rozmawiać między sobą. Dziś te informacje krążą po Facebooku, gdzie giną w
feedzie, nie tworzą trwałego archiwum, a dyskusja kibiców jest rozproszona. Klub
nie ma własnego kanału, nad którym miałby kontrolę.

Insight: wartością nie jest sam „newsfeed", lecz skupienie **aktualności i forum
kibiców w jednym, klubowym miejscu** — zamiast rozpraszania ich po social mediach.
Dodatkowym wyróżnikiem jest automatyczne generowanie wpisów aktualności z surowych
danych meczowych (publikowane po akceptacji administratora), co odciąża osobę
prowadzącą komunikację klubu.
...
Model wielodostępowy z logowaniem przez zewnętrznego dostawcę (OAuth —
Google/Facebook). Brak haseł zarządzanych przez aplikację.

Role i uprawnienia:
- **Gość (niezalogowany)** — może czytać aktualności. Nie ma dostępu do forum
...
MVP = pełny pipeline aktualności z AI (forum jest fast-followem, poza MVP).
Przepływ główny: admin loguje się → wkleja dane meczowe → system generuje
propozycję wpisu → admin przegląda/poprawia/akceptuje → wpis publikowany w
publicznych aktualnościach → kibic/gość go czyta.

### Primary
- Administrator publikuje wpis aktualności wygenerowany z surowych danych meczowych
  przez ścieżkę „dane → AI → akceptacja → publikacja", a kibic/gość czyta go w
  publicznej zakładce. Pipeline działa end-to-end.
...
- Żaden wygenerowany wpis nie trafia do publicznych aktualności bez wyraźnej
  akceptacji administratora — AI nigdy nie publikuje samodzielnie.
- Dane logowania kibiców pochodzą od zewnętrznego dostawcy (OAuth); aplikacja nie
  przechowuje haseł.
...
- FR-002: Kibic może zalogować się przez zewnętrznego dostawcę (OAuth — Google/Facebook). Priority: nice-to-have
  > Socrates: Potrzebne tylko do interakcji (reakcje/komentarze) i forum — wszystkie nice-to-have. Świadomie poza MVP.

### Aktualności — generowanie z AI
- FR-003: Administrator może wprowadzić surowe dane meczowe luźnym tekstem (skład, strzelcy, wynik, przebieg) wraz z opisem pożądanego tonu i stylu wpisu. Priority: must-have
  > Socrates: Rozważono formularz strukturalny dla lepszej jakości generacji. Rozstrzygnięcie: luźny tekst — administrator pisze jak chce, w tym wskazuje ton/styl (np. żartobliwie po wygranej, rzeczowo po porażce). Elastyczność ważniejsza niż sztywne pola.
- FR-004: Administrator może wygenerować propozycję wpisu aktualności na podstawie wprowadzonych danych meczowych. Priority: must-have
  > Socrates: Rozważono „szablon zamiast AI". Rozstrzygnięcie: AI to rdzeń wartości — administrator może wskazać ton i styl wpisu, czego szablon nie osiągnie. Zostaje.
- FR-005: Administrator może przejrzeć i edytować wygenerowaną propozycję przed publikacją. Priority: must-have
  > Socrates: Brak kontrargumentu — możliwość poprawy propozycji jest warunkiem sensownej akceptacji. Zostaje.
- FR-006: Administrator może zaakceptować i opublikować propozycję albo ją odrzucić. Priority: must-have
...
- FR-007: Administrator może utworzyć wpis aktualności ręcznie, bez generowania. Priority: must-have
  > Socrates: Rozważono usunięcie z MVP (skoro pipeline AI działa). Rozstrzygnięcie: zostaje — komunikaty nie-meczowe (treningi, wydarzenia, ogłoszenia) nie mają danych meczowych i wymagają ścieżki ręcznej.
- FR-008: Administrator może edytować opublikowany wpis aktualności. Priority: must-have
  > Socrates: Brak kontrargumentu — poprawa literówek/błędów po publikacji jest oczywista. Zostaje.
- FR-009: Administrator może usunąć wpis aktualności. Priority: must-have
  > Socrates: Brak kontrargumentu — wycofanie błędnego wpisu jest konieczne. Zostaje.
- FR-010: Gość/kibic może przeglądać i czytać opublikowane aktualności. Priority: must-have
...
- FR-014: Kibic może założyć temat na forum. Priority: nice-to-have
  > Socrates: Forum świadomie odłożone jako fast-follow (Faza 3). Poza MVP.
...
w publicznej zakładce aktualności, widocznej dla każdego bez logowania. Reguła
ta dotyczy wpisów generowanych; wpisy tworzone ręcznie pomijają etap generowania,
ale podlegają tej samej publikacji.
...
- Logowanie odbywa się wyłącznie przez zewnętrznego dostawcę (OAuth); aplikacja nie
  przechowuje haseł i ogranicza dane o kibicu do minimum niezbędnego do jego
  identyfikacji.
...
- **Forum i interakcje kibiców w MVP** — logowanie kibiców (OAuth), reakcje
  („łapki"), komentarze pod aktualnościami oraz forum są poza pierwszą wersją
  (fast-follow / nice-to-have). MVP obsługuje wyłącznie publikację i czytanie
  aktualności.
...
Brak otwartych kwestii blokujących — wszystkie gray-areas rozstrzygnięto podczas
shapingu (patrz `gray_areas_resolved` w checkpoint oraz blockquote'y `> Socrates:`
przy FR-ach). Kontrola jakości: `accepted`.
