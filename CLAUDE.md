...
- When `/10x-bootstrapper` lands (Lesson 3), scaffold the Angular app into `frontend/`
  and the Spring app into `backend/` rather than into new top-level directories named
  after `project_name`. Reconcile the scaffold target with this layout before running it.
